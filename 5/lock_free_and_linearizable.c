...
void enqueue(struct queue *q, int data) {
    ...
    while ( TRUE ) {
        /* enqueue node */
        /* change next pointer of old tail to new element only if it is NULL */
        tail = q->tail; 
        if (! __sync_bool_compare_and_swap(&(tail->next), NULL, new) ) {
            continue;
        }
        
        /* forward tail pointer: if it is alfready moved, ignore */
        __sync_bool_compare_and_swap(&(q->tail), tail, new);
        break;
    }
}
